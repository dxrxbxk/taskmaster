...
namespace sm {
	...
	class client_manager final {
		...
		public:
			...
			/* add */
			auto add(sm::client&& client) -> void {
				_clients.emplace(client.socket().fd(), static_cast<sm::client&&>(client));
			}

			/* add */
			auto add(sm::socket&& socket) -> void {
				_clients.emplace(socket.fd(), static_cast<sm::socket&&>(socket));
			}
			...
			/* remove */
			auto remove(const sm::client& client) -> void {
				_clients.erase(client.socket().fd());
			}
